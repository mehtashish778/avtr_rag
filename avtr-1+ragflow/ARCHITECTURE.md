...
flowchart TB
    ...
    G3 --> B1["RAGFlow API<br/>POST /api/v1/retrieval"]
    B1 --> B2["RAGFlow retrieval engine<br/>dataset filter + vector ranking + Top-K"]
    B2 --> B3[("RAGFlow dataset<br/>parsed chunks + vector index")]
    B3 -->|ranked chunks and similarity| G4["_normalize_ragflow()<br/>context + references"]
    ...
    D[("Breast-cancer documents")] --> G6["rag_admin.ingest_ragflow()"]
    G6 --> B4["RAGFlow document API<br/>POST /datasets/{id}/documents<br/>POST /datasets/{id}/chunks"]
    B4 --> B3
...
- Blue: original RAGFlow functions.
- Green: functions created to integrate AVTR-1 with RAGFlow.
...
`http://127.0.0.1:9380/api/v1/retrieval` when RAGFlow runs beside AVTR-1; use the
configured reachable RAGFlow host otherwise.
