...
flowchart TB
    ...
    G3 --> B1["LightRAG API<br/>POST /query"]
    B1 --> B2["LightRAG query pipeline<br/>mode=naive, context-only, Top-K"]
    B2 --> B3[("LightRAG workspace<br/>chunks + vectors + graph/KV stores")]
    B3 -->|chunks and references| G4["_normalize_lightrag()<br/>context + references"]
    ...
    D[("Breast-cancer documents")] --> G6["rag_admin.ingest_lightrag()"]
    G6 --> B4["LightRAG<br/>POST /documents/text"]
    B4 --> B3
...
- Blue: original LightRAG functions.
- Green: functions created to integrate AVTR-1 with LightRAG.
