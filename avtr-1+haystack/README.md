...
```bash
git clone https://github.com/mehtashish778/avtr_rag.git
cd avtr_rag/avtr-1+haystack
...
Realtime conversation engine. Hayhooks exposes native Haystack indexing and
query pipelines backed by Qdrant; there is no AVTR-owned FastAPI service.
Retrieval runs outside the WebSocket listener, returns normalized
context/references, and suppresses stale follow-up responses when the user
interrupts.

The implementation is retrieval-only. Haystack embeds, retrieves, and reranks
source passages; OpenAI Realtime remains the only answer and speech generator.
See [ARCHITECTURE.md](ARCHITECTURE.md) for the component flow.

### Configure and start locally

1. Copy `.env.example` to the untracked `.env` and configure its `HAYSTACK__*`
   and `RAG__*` values. Keep Qdrant and Hayhooks bound to loopback.
2. Start Qdrant at `HAYSTACK__QDRANT_URL` (the default is
   `http://127.0.0.1:6333`).
3. Start Hayhooks and verify that both native pipelines deployed:

```bash
pixi run -e haystack hayhooks-service
curl -fsS http://127.0.0.1:1416/status
curl -fsS http://127.0.0.1:1416/openapi.json
```

Hayhooks provides these application endpoints:

- `POST /avtr_haystack_index/run` for `.txt` and `.md` uploads
- `POST /avtr_haystack_query/run` for dense retrieval and reranking
- `GET /status` for service health and pipeline deployment status
- `GET /openapi.json` for the generated API contract

### Ingest and verify

The included evaluation corpus is under
`knowledge_base/documents/breast_cancer/`. Re-ingestion is idempotent because
the native `DocumentWriter` uses `DuplicatePolicy.OVERWRITE`.

```bash
pixi run -e haystack haystack-ingest
pixi run -e haystack haystack-smoke
pixi run -e haystack haystack-test
```

To ingest a different approved directory or choose the smoke-test question:

```bash
pixi run -e haystack python scripts/rag_admin.py ingest \
  --documents-dir /path/to/approved/documents
pixi run -e haystack python scripts/rag_admin.py smoke \
  --query "What are common signs of breast cancer?"
```

Start the avatar with `pixi run -e streamer interactive-demo` after Qdrant and
Hayhooks are healthy. Set `RAG__ENABLED=false` to restore the original non-RAG
conversation behavior. For Slurm deployment on MBZUAI CIAI, follow
[RAG_CIAI.md](RAG_CIAI.md); the renderer must never run on a login node.

Indexes, caches, logs, and generated results belong under
`/l/users/ashish.mehta/avtr_rag-runtime` and must remain untracked. Never add
identifiable patient data, medical conversations, API keys, or TURN credentials
to the repository.
