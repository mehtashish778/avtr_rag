...
RagBackend = Literal["txtai", "lightrag", "ragflow", "haystack"]

DEFAULT_BACKEND: RagBackend = "haystack"
DEFAULT_URLS: dict[RagBackend, str] = {
    "txtai": "http://127.0.0.1:8001",
    "lightrag": "http://127.0.0.1:9621",
    "ragflow": "http://127.0.0.1:9380",
    "haystack": "http://127.0.0.1:1416",
}
...
class RagSettings(BaseModel):
    ...
    mode: str = "naive"
    initial_top_k: int = Field(default=10, ge=1, le=100)
    top_k: int = Field(default=3, ge=1, le=20)
    timeout_seconds: float = Field(default=5.0, gt=0, le=120)
    max_context_tokens: int = Field(default=1200, ge=100, le=32000)
...
class RagClient:
    """Queries the configured retrieval backend and normalizes its response."""
    ...
    async def query(self, query: str) -> dict[str, object]:
        ...
        try:
            ...
            _LOGGER.exception("Unexpected RAG query failure backend=%s", self.settings.backend)
    ...
    async def _request(self, query: str) -> httpx.Response:
        ...
        if settings.backend == "haystack":
            return await self._http.post(
                "/avtr_haystack_query/run",
                headers=headers,
                json={
                    "query": query,
    ...
    def _normalize(self, payload: Any) -> tuple[str, list[dict[str, object]]]:
        ...
        if self.settings.backend == "haystack":
            return self._normalize_haystack(payload)
    ...
    def _normalize_txtai(self, payload: Any) -> tuple[str, list[dict[str, object]]]:
        ...
        for rank, row in enumerate(rows[: self.settings.top_k], start=1):
            ...
            references.append({"rank": rank, "source": source, "score": row.get("score")})
    ...
    def _normalize_ragflow(self, payload: Any) -> tuple[str, list[dict[str, object]]]:
        ...
        for rank, row in enumerate(rows[: self.settings.top_k], start=1):
            if not isinstance(row, dict):
                continue
            text = str(
                row.get("content_with_weight") or row.get("content") or row.get("text") or ""
            ).strip()
            source = str(
            ...
            if text:
                chunks.append(f"[Source {source}]\n{text}")
            references.append({"rank": rank, "source": source, "score": row.get("similarity")})
        return self._trim("\n\n".join(chunks)), references

    def _normalize_haystack(self, payload: Any) -> tuple[str, list[dict[str, object]]]:
        if not isinstance(payload, dict):
            raise ValueError("Hayhooks response is not an object")
        rows = payload.get("documents", [])
        if not isinstance(rows, list):
            raise ValueError("Hayhooks response does not contain documents")

        chunks: list[str] = []
        references: list[dict[str, object]] = []
        for rank, row in enumerate(rows[: self.settings.top_k], start=1):
            if not isinstance(row, dict):
                raise ValueError("Haystack document is not an object")
            text = str(row.get("content") or "").strip()
            if not text:
                continue
            nested_meta = row.get("meta")
            if nested_meta is not None and not isinstance(nested_meta, dict):
                raise ValueError("Haystack document metadata is not an object")
            # Haystack's native Document.to_dict() flattens metadata into the
            # document mapping. Accept nested metadata as well for compatibility.
            raw_meta = nested_meta if isinstance(nested_meta, dict) else row
            document_id = str(
                raw_meta.get("document_id") or raw_meta.get("source_id") or ""
            ).strip()
            source = str(
                document_id
                or raw_meta.get("file_name")
                or raw_meta.get("file_path")
                or row.get("id")
                or f"result-{rank}"
            )
            block = f"[Source {source}]\n{text}"
            candidate = "\n\n".join([*chunks, block])
            if self._trim_to_token_budget(candidate) != candidate:
                continue
            chunks.append(block)
            references.append(
                {
    ...
    def _trim_to_token_budget(self, context: str) -> str:
        context = context.strip()
        if ceil(len(context.encode("utf-8")) / 4) <= self.settings.max_context_tokens:
            return context
        blocks = context.split("\n\n")
        kept: list[str] = []
        for block in blocks:
            candidate = "\n\n".join([*kept, block])
            if ceil(len(candidate.encode("utf-8")) / 4) > self.settings.max_context_tokens:
                break
            kept.append(block)
        return "\n\n".join(kept)
