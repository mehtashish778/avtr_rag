...
async def ingest_txtai(http: httpx.AsyncClient, files: list[Path], auth: dict[str, str]) -> None:
    payload = [
        {
            "id": path.stem,
...
async def ingest_lightrag(http: httpx.AsyncClient, files: list[Path], auth: dict[str, str]) -> None:
    for path in files:
...
async def ingest_ragflow(
    ...
    uploads = [
        ("file", (path.name, path.read_bytes(), "text/plain; charset=utf-8")) for path in files
    ]
    response = await http.post(
...
async def ingest_haystack(http: httpx.AsyncClient, files: list[Path], auth: dict[str, str]) -> None:
    uploads = [
        (
            "files",
            (
...
async def ingest(directory: Path) -> int:
    ...
    async with httpx.AsyncClient(
        ...
        elif settings.backend == "haystack":
            await ingest_haystack(http, files, auth)
        else:
