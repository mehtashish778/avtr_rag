# AVTR-1 + Haystack on CIAI: setup, start, stop, and restart

This runbook documents the AVTR-1 + Haystack setup on the MBZUAI CIAI cluster.
...
Request one GPU explicitly. The three-hour walltime below is suitable for an
interactive session; adjust it to the current partition limits and workload:
...
First start Qdrant and Hayhooks as described in
[`RAG_CIAI.md`](RAG_CIAI.md), then start the GPU renderer and streamer:
...
```text
/home/ashish.mehta/avtr_rag/avtr-1+haystack/.env
```
...
Run these only when building the environment or storage from scratch.
...
### Allocation ends at its requested walltime

Slurm jobs expire at their requested walltime. Files, model artifacts, Pixi
environments, and caches under `/l/users/ashish.mehta` persist; request a new
allocation and repeat the quick restart checklist.
